...
function onPhotoFileSuccess(imageData) { 
  ...
  //alert('Foto File tomada');
  //alert(imageData);
  
  //fileSystem.root.getDirectory("DirFotos",{create: true, exclusive: false}, function(dir){alert("Created dir "+dir.name);},function(error){alert("Error creating directory "+ fileErrorCode(error.code));});
...
function guarda_evidencia_foto(latitud, longitud){
  ...
  if(evidencia_actual.TIPO_EVIDENCIA==1 || evidencia_actual.TIPO_EVIDENCIA==2 || evidencia_actual.TIPO_EVIDENCIA==3 || evidencia_actual.TIPO_EVIDENCIA==4 || evidencia_actual.TIPO_EVIDENCIA==5){
    DATOS_borra_evidencia_guia(evidencia_actual, function(result_guardado) {
      DATOS_guardar_evidencia_guia(evidencia_actual, function(result_guardado) {
        ...
        //cargado 1, cargado 2
        if(evidencia_actual.TIPO_EVIDENCIA==2  || evidencia_actual.TIPO_EVIDENCIA==4){
...
function capturePhotoWithFile(idgde, tipo_evidencia) {
  ...
  navigator.camera.getPicture(onPhotoFileSuccess, onFail, { quality: 90, destinationType: Camera.DestinationType.FILE_URI,targetWidth: 1920,
  targetHeight: 1080 }); 
} 
