...
function DATOS_seleccionar_evidencias_FOTOS_Por_enviar(estado, callback){
	...
	//alert("entro a guardar gde");
		this.db.transaction(function(tr) {
			tr.executeSql("SELECT e.*,e.rowid,  strftime('%d-%m-%Y %H:%M', e.FECHA_EVIDENCIA) as fecha_format FROM GDE_EVIDENCIA e WHERE e.ENVIADO_FOTO=? AND e.GDE_COD_DESPACHADOR=?  ORDER BY FECHA_EVIDENCIA ASC", [estado, Obtener_dato_local("rut_activo")], function(tr, rs) {
				var n = rs.rows.length;
				if(n==0){
					typeof callback == "function" && callback(-1);
				}else{
